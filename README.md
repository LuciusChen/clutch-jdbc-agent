...
Otherwise it retires the logical connection, or only the metadata or bulk session
for a fetch on one of those. Cancellation itself is driver-dependent and may block.
...
The agent no longer runs requests on one global synchronous lane. The stdin loop parses requests and hands them to a small request pool. The dispatcher uses independent per-connection foreground and metadata locks, so ordinary queries and metadata may run in parallel without allowing two operations to race on either JDBC session. On Oracle a third, lazily opened bulk session carries schema-wide listings (`get-tables`, `get-sequences`, `get-procedures`, `get-functions`, and `get-indexes`/`get-triggers` without a `table`), so a listing that takes seconds on a large schema cannot hold the metadata session while an interactive lookup such as `get-primary-keys` waits; other products keep two sessions. A bulk session that a connection failure has made unusable is dropped and the listing retried once on a fresh one, without touching the metadata session, and a schema change drops it so the next listing reopens it with the remembered schema. The third session counts against the account's session limits; if Oracle refuses it, that connection stops using a bulk session and its schema-wide listings run on the metadata session, as on other products. Schema changes and disconnect acquire all locks in foreground-then-metadata-then-bulk order. If an idle timeout or Oracle `ORA-12592` makes the metadata session unsafe, the agent replaces only that session, restores the remembered schema, and retries the metadata request once; a failed retry or schema restore invalidates that replacement for the next request instead of reusing uncertain state. A metadata or bulk session idle for `validate-after-idle-seconds` is checked with `Connection.isValid(3)` before the request and replaced the same way when the check fails: a NAT or firewall that drops an idle connection leaves its socket silent, so a request on it would wait out the network timeout, and Clutch, whose request timeout is no longer, would retire the whole logical connection and its open transaction first. The primary transaction is untouched. `cancel` is the deliberate exception to serialization: it can arrive while `execute` or `fetch` is running, locate the live `Statement`, and call `Statement.cancel()` without tearing down the whole session.
